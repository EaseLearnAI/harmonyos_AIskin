...
本目录属于提交 `4359098` 附近的第一轮整改，不能用于证明后续共享组件重建后的最终外观。当前前端验收见 [frontend-evidence](../frontend-evidence/README.md)。
...
这些是开发验证证据，不是 Mate 60 / Pura 80 Pro+ / Mate X5 真机截图，也不是 AGC 通过证明。截图拍摄于最终小幅调整辅助文字颜色、关闭按钮尺寸及移除方案按钮装饰符之前；此后页面与共享组件又进行了整体迁移，这组布局不代表当前源码。最终签名发布包仍需重新取证。

登录预填只存在于临时调试副本，未纳入源码和正式包，含账号的登录截图未复制到这里。

`onboarding-release-light.jpeg` 和 `login-release-light.jpeg` 是第一轮未签名 Release 包安装启动后的截图，登录输入框均为空。
